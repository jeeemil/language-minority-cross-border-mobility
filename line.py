""" This script is using multiprocessing to create vector lines from twitter data based on a list of users.
It has been created with the help of scripts by Håvard Aagesen and Tuomas Väisänen"""
...
import language_detection.get_lang_user_list as get_lang
...
def shift_userLines(user):
    line_data = gpd.GeoDataFrame(columns=['geometry', 'user_id','orig_country',  'dest_country', 'orig_time' , 'dest_time',  'duration', 'distance_km'], geometry='geometry')
    ...
    user1['orig_country'] = user1['country']
    
    # create shifted columns
    user1['geo2'] = user1['geometry'].shift(-1)
    user1['dest_country'] = user1['country'].shift(-1)
    user1['dest_time'] = user1['orig_time'].shift(-1)
    user1['cb_move'] = user1.apply(lambda x: str(x['country'])+'-'+str(x['dest_country']) ,axis=1)
...
user_list = get_lang.get_user_list(lang)
...
if __name__=='__main__':
    ...
    # Creating the vector lines for all set users
    result = a_pool.map(shift_userLines, tqdm(user_list))

    # Closing pool
    a_pool.close()
    a_pool.join()

    # Concating results to a DataFrame
    data = pd.DataFrame()
    data = pd.concat(result)
    print("Number of movements: " + str(len(data)))
    print("Number of users: " + str(len(data.user_id.unique())))
    # Excluding vectors between two points in the same country
    only_cb = data[data.orig_country != data.dest_country]
    print("Number of cross-border movements: " + str(len(only_cb)))
    print("Number of users: " + str(len(only_cb.user_id.unique())))
    
    # Creating an empty DataFrame to include only cross-border movements from or to Finland
    only_domestic_cb = pd.DataFrame()
    
    # Dropping all cross-border movements that does not regard Finland.
    for i, row in only_cb.iterrows():
        if (row['orig_country'] != 'Finland' and row['dest_country'] != 'Finland'):
            pass
        else:
            only_domestic_cb = only_domestic_cb.append(row)

    print("Number of cross-border movements from Finland to other country: " + str(len(only_domestic_cb)))
    print("Number of users: " + str(len(only_domestic_cb.user_id.unique())))

    # Create pickle
    #only_domestic_cb.to_pickle(f'{lang}_all_cb_movements.pkl')  

    # For creating a geodataframe
    #geo = gpd.GeoDataFrame(only_domestic_cb, crs="EPSG:4326", geometry='geometry')
    #geo.to_file(f'{lang}_all_cb_lines.geojson', driver='GeoJSON')
